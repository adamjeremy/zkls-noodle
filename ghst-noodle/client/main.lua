                ...
                BusinessZone = exports['qb-target']:AddBoxZone(Config.Job..k..r, s.coords, s.length, s.width, {
                    ...
                    maxZ = s.coords.z + 1,
                        ...
                        options = {
                            {
                                type = 'client',
                                event = 'ghst-noodle'..Config.Job..':client:Open'..k,
                                icon = s.info.icon,
                                label = s.info.label,
                                job = Config.Job
                            },
                ...
                StashZones = exports['qb-target']:AddBoxZone('Stash '..Config.Job..r, s.coords, s.length, s.width, {
                    ...
                    maxZ = s.coords.z + 1,
                        ...
                        options = {
                            {
                                action = function()
                                    TriggerEvent('ghst-noodle'..Config.Job..':OpenStash', r)
                                end,
                                icon = s.info.icon,
                                label = s.info.label,
                                job = Config.Job
                ...
                CounterZones = exports['qb-target']:AddBoxZone('Tray '..Config.Job..r, s.coords, s.length, s.width, {
                    ...
                    maxZ = s.coords.z + 1,
                        ...
                        options = {
                            {
                                action = function()
                                    TriggerEvent('ghst-noodle'..Config.Job..':OpenTray', r)
                                end,
                                icon = s.info.icon,
                                label = s.info.label,
                            },
                ...
                SinkZones = exports['qb-target']:AddBoxZone('Sink '..Config.Job..r, s.coords, s.length, s.width, {
                    ...
                    maxZ = s.coords.z + 1,
                        ...
                        options = {
                            {
                                type = 'client',
                                event = 'ghst-noodle'..Config.Job..'WashHands',
                                icon = s.info.icon,
                                label = s.info.label,
                                job = Config.Job
                            },
...
RegisterNetEvent('ghst-noodle'..Config.Job..':OpenTray', function(stashID)
...
RegisterNetEvent('ghst-noodle'..Config.Job..':OpenStash', function(stashID)
...
RegisterNetEvent('ghst-noodle'..Config.Job..'WashHands', function()
...
RegisterNetEvent('ghst-noodle'..Config.Job..':client:MakeItem', function(data)
    local Item = data.item
    local Required = data.required
    local ItemID = data.itemID
    local Emote = data.craftemote

    QBCore.Functions.TriggerCallback('ghst-noodle'..Config.Job..':server:HasItems', function(hasItems)
        if hasItems then
            ...
            exports['ps-ui']:Circle(function(success)
                if success then
                    QBCore.Functions.Progressbar('business_food', 'Making a '..sharedItems[Item].label, (Config.Times.Food * 1000), false, true, {
                        ...
                        QBCore.Functions.TriggerCallback('ghst-noodle'..Config.Job..':server:GetItem', function(isMade)
                            if isMade then
