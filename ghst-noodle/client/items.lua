...
---------------------------------------------
-- EAT EVENT --
RegisterNetEvent("ghst-noodle"..Config.Job..":client:Eat", function(item, emote, time, hunger)
    ...
    if hasItem then
        ...
        QBCore.Functions.Progressbar('eating_something', 'Eating '..sharedItems[item].label, (time * 1000), false, true, {
            ...
            QBCore.Functions.TriggerCallback('ghst-noodle'..Config.Job..':server:UseItem', function(hasEaten)
                if hasEaten then
                    TriggerServerEvent('ghst-noodle'..Config.Job..':server:addHunger', QBCore.Functions.GetPlayerData().metadata["hunger"] + hunger)
...
-- DRINK EVENT --
RegisterNetEvent("ghst-noodle"..Config.Job..":client:Drink", function(item, emote, time, thirst)
    local hasItem = QBCore.Functions.HasItem(item)
    ...
    if hasItem then
        ...
        QBCore.Functions.Progressbar('drink_something', 'Drinking '..sharedItems[item].label, (time * 1000), false, true, {
            ...
            TriggerEvent('animations:client:EmoteCommandStart', {'c'})
            QBCore.Functions.TriggerCallback('ghst-noodle'..Config.Job..':server:UseItem', function(hasDrank)
                if hasDrank then
                    TriggerServerEvent('ghst-noodle'..Config.Job..':server:addThirst', QBCore.Functions.GetPlayerData().metadata["thirst"] + thirst)
...
-- DRINK ALCOHOL --
RegisterNetEvent("ghst-noodle"..Config.Job..":client:DrinkAlcohol", function(item, emote, time, thirst)
    local hasItem = QBCore.Functions.HasItem(item)
    local Emote = tostring(emote)
...
-- DRINK COFFEE --
RegisterNetEvent("ghst-noodle"..Config.Job..":client:DrinkCoffee", function(item, emote, time, thirst)
    local hasItem = QBCore.Functions.HasItem(item)
    local Emote = tostring(emote)
